...
public class FireCar extends Car{
    ...
    /* 2. soundHorn() 오버라이딩 */
    /* @Override 어노테이션
     *  JDK 1.5부터 추가된 문법으로 오버라이딩 성립 요건을 체크하여 성립되지 않는 경우 컴파일 에러를 발생시킨디ㅏ.
    ...
    public void soundHorn(){
        if(isRunning()){
            System.out.println("빠아아아아아아ㅏ아앙~~~~!!~!~!!~!~!~!");
        } else {
            System.out.println("소방차가 앞으로 갈 수 없습니다~ 비켜비켜~~ 다비켜~");
        }
    }

    /* 3. 물뿌리는 기능 추가 */
    public void sprayWater(){
        System.out.println("불난 곳을 발견했습니다. 물을 뿌려요~~~~~ 뿌려!!");
    }
